...
\begin{itemize}
  \item \textbf{Warren \& Brandeis (1890)}: \emph{``The right to be alone''} -- uno dei saggi più influenti nella storia del diritto americano; gli autori cercavano di trovare una base giuridica per proteggere la privacy nell'ambito delle leggi e dei princìpi già esistenti. Esempi di applicazione: il diritto di una persona di scegliere la solitudine lontano dall'attenzione degli altri; il diritto di essere immune dallo scrutinio o dall'essere osservati in contesti privati, come la propria abitazione.
  \item \textbf{Godkin (1880)}: \emph{``Nothing is better worthy of legal protection than private life, or the right of every man to keep his affairs to himself, and to decide for himself to what extent they shall be the subject of public observation and discussion.''} -- accesso limitato alle informazioni personali.
  \item \textbf{Bok (1989)}: \emph{``The condition of being protected from unwanted access by others -- either physical access, personal information, or attention.''} -- protezione da accessi indesiderati.
  \item \textbf{Westin \& Blom-Cooper (1970)}: \emph{``Privacy is the claim of individuals, groups, or institutions to determine for themselves when, how, and to what extent information about them is communicated to others.''} -- controllo sull'informazione.
  \item \textbf{Fried (1968)}: \emph{``Privacy is not simply an absence of information about us in the minds of others; rather it is the control we have over information about ourselves.''} -- controllo sulle proprie informazioni.
  \item \textbf{FIPS PUB 41} (NIST -- \emph{Computer Security Guidelines for Implementing the Privacy Act of 1974}): \emph{``The right of an entity to determine the degree to which it will interact with its environment, including the degree to which the entity is willing to share its personal information with others.''}
  \item \textbf{ISO}: Il diritto di un individuo a controllare o influenzare quali informazioni collegate a loro possono essere collezionate e salvate.
\end{itemize}
\end{definizione}
...
\begin{enumerate}
  ...
  \item Funzionare \emph{anonimamente} nella societ\`{a}.
\end{enumerate}
\end{definizione}
...
Il fenomeno della \textbf{datafication}: tutto viene convertito in dati (carte di credito, registri di chiamate, dati sanitari, email, social, DNA, etc.). Questo crea un trade-off: pi\`{u} privacy $\Leftrightarrow$ meno comodit\`{a}.
...
L'intuizione è: prima di verificare la k-anonymity su tutti gli attributi insieme (operazione costosa), verifica ogni singolo attributo da solo. Se già un solo attributo preso individualmente viola la k-anonymity, allora qualunque combinazione che lo includa la violerà sicuramente -- aggiungere attributi crea gruppi ancora più piccoli. Questo taglia enormemente lo spazio di ricerca, esattamente come l'algoritmo Apriori nel frequent itemset mining: se un insieme piccolo è problematico, tutti i suoi soprainsieme lo saranno.
\end{nota}
...
\]
Poich\'{e} PT contiene solo malati, $P(I_p \mid p \in PT) = 1$; per chi non \`{e} in PT la probabilit\`{a} che sia diabetico \`{e} $b_r \cdot \frac{|T| - |PT|}{|T| - |PT|} = b_r$. Sostituendo (eq.~1):
\[
b_0 = \delta \cdot \frac{|T|(1 - b_r)}{|T| - |PT|} + \frac{b_r \cdot |T| - |PT|}{|T| - |PT|}
\]
dove $\delta = P(p \in PT \mid GT)$.
...
La sensibilità globale risponde alla domanda: ``Di quanto può cambiare al massimo la risposta alla mia query se aggiungo o rimuovo un singolo record?'' Due esempi chiari:
\begin{itemize}
  \item Query \textbf{COUNT} (``quanti record ci sono?''): aggiungere o rimuovere un record cambia il risultato esattamente di 1 $\Rightarrow GS_1 = 1$.
  \item Query \textbf{SUM} (``qual è la somma degli stipendi?''): aggiungere un record con stipendio $s$ cambia la somma di $s$; nel caso peggiore $s$ è il valore massimo del dominio $\Rightarrow GS_1 = \max(\text{stipendio})$.
\end{itemize}
Il meccanismo di Laplace aggiunge rumore proporzionale a $GS_1/\varepsilon$: più la query è sensibile (grande $GS_1$) o più privacy vuoi (piccolo $\varepsilon$), più grande sarà il rumore aggiunto.
\end{nota}
...
\begin{itemize}
  \item $GS_1(q) = 1$ (aggiungere/rimuovere un record cambia il conteggio di 1).
...
\]
Preserva $\varepsilon/2$-DP (o $\varepsilon$-DP a seconda della formulazione).
\end{definizione}
...
I tre teoremi di composizione si leggono così nella pratica:
\begin{itemize}
  \item \textbf{Post-processing}: puoi elaborare liberamente l'output privato (calcolare medie, fare grafici, ecc.) senza perdere ulteriore privacy -- perché non tocchi più i dati originali.
  \item \textbf{Composizione sequenziale}: se fai due query, ognuna con il suo budget $\varepsilon$, il budget totale consumato è $\varepsilon_1 + \varepsilon_2$. È come un conto corrente: ogni query preleva una quota.
  \item \textbf{Composizione parallela}: se invece le due query riguardano partizioni disgiunte del dataset (es. uomini e donne separatamente), il budget consumato è solo $\max(\varepsilon_1, \varepsilon_2)$ -- non si sommano, perché ogni individuo appare in una sola partizione.
\end{itemize}
\end{nota}
...
\textbf{Confronto Entropia}: il Gaussiano ha entropia $\frac{1}{2}\log(2\pi e\sigma^2)$; il Laplace ha entropia $\ln(\sqrt{2}e\sigma)$. Il Gaussiano ha entropia minore (meno rumore) a parit\`{a} di $\sigma$.
...
\end{enumerate}
Probabilit\`{a}: $P(\text{Sì}|\text{vero=Sì}) = 3/4$; $P(\text{No}|\text{vero=Sì}) = 1/4$.

Il Randomized Response preserva $\log(3)$-DP (LDP).

\textbf{Prova}: Il rapporto massimo \`{e} $\frac{3/4}{1/4} = 3 = e^{\log 3}$.
\end{definizione}
...
\textbf{Oblivious Polynomial Evaluation}: il sender ha un polinomio $Q$ di grado $k$; il receiver ha un valore $z$; il receiver ottiene $Q(z)$ senza che il sender sappia $z$ e senza che il receiver impari i coefficienti di $Q$. Overhead: $O(k)$ esponenziali.
...
\begin{itemize}
  \item \textbf{Crittografia funzionale}: invece di dover decriptare l'intero messaggio, una chiave speciale permette di calcolare solo una funzione specifica del testo in chiaro (es. la media degli stipendi di tutti, senza vedere i singoli valori). Generalizza la crittografia tradizionale a chiave pubblica.
  \item \textbf{Crittografia omomorfica}: puoi eseguire calcoli direttamente sui dati \emph{cifrati} -- il risultato decifrato coincide con quello che avresti ottenuto operando sui dati in chiaro. Concettualmente potente, ma attualmente è molto lenta (ordini di grandezza rispetto al calcolo normale); usata in scenari dove la privacy è prioritaria rispetto alle prestazioni.
  \item \textbf{Federated learning}: ogni dispositivo (es. il tuo smartphone) allena il modello ML localmente sui propri dati e manda solo il \emph{gradiente} (ovvero ``in che direzione modificare i parametri del modello'') al server centrale. Il server aggrega i gradienti di tutti gli utenti e aggiorna il modello globale. I dati grezzi non lasciano mai il dispositivo locale -- è la tecnica usata ad esempio da Google per migliorare la tastiera Gboard senza accedere ai tuoi messaggi.
\end{itemize}
\end{nota}
...
\begin{itemize}
  ...
  \item \textbf{Azioni}: come vengono impiegate le decisioni su input nuovi.
  \item \textbf{Feedback}: registrazione delle reazioni degli utenti.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Indipendenza (Demographic Parity)}: il modello deve approvare la stessa percentuale di richieste in ogni gruppo -- es. stessa percentuale di prestiti concessi a uomini e donne, indipendentemente dal merito individuale. Problema: può portare a assumere ``a caso'' dal gruppo svantaggiato.
  \item \textbf{Separazione (Equal Error Rates)}: il modello può sbagliare, ma deve sbagliare con la \emph{stessa frequenza} per tutti i gruppi. Esempio: il sistema COMPAS deve avere lo stesso tasso di falsi positivi (etichettare come ``ad alto rischio'' chi non lo è) per bianchi e afroamericani.
  \item \textbf{Sufficienza (Calibration)}: se il modello predice ``70\% di rischio di recidiva'', quel 70\% deve essere ugualmente valido per bianchi e afroamericani -- il punteggio deve essere ugualmente affidabile per tutti i gruppi, indipendentemente dall'attributo sensibile.
\end{itemize}
Il \textbf{teorema di incompatibilità} dice che, in quasi tutti i contesti reali (dove $A$ e $Y$ sono correlati), non si possono soddisfare contemporaneamente tutti e tre i criteri: bisogna scegliere quale garanzia è più importante e accettare di violare le altre. Non esiste una definizione universale di ``fairness''.
\end{nota}
