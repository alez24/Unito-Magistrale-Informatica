...
\pagestyle{fancy}
\fancyhf{}
\rhead{\small Convertitore Italiano $\rightarrow$ Italiano-Yoda}
\lhead{\small Elaborato di Linguaggi e Grammatiche Formali}
\cfoot{\thepage}
...
\title{
    ...
    {\large \textit{Linguaggi Formali e Compilatori / Linguistica Computazionale}}\\[1cm]
    \rule{\linewidth}{1.5pt}\\[0.5cm]
    {\LARGE \textbf{Convertitore Lessicale Italiano $\rightarrow$ Italiano-Yoda}}\\[0.3cm]
...
conversione automatica di frasi in lingua italiana nel cosiddetto \textit{Italiano-Yoda} (IT-YO),
ovvero una variante artificiale dell'italiano che riproduce il caratteristico ordine sintattico
usato dal personaggio di Yoda nella saga di Guerre Stellari: complemento oggetto, soggetto,
verbo (ordine OVS/XSV). Il sistema è basato su una grammatica context-free in Forma Normale
di Chomsky (CNF), su un parser CKY implementato in Python, e su un modulo di trasformazione
ricorsiva dell'albero di derivazione. Vengono discussi la modellazione della grammatica, le
strutture dati adottate, l'algoritmo di parsing, la trasformazione sintattica e i limiti del
sistema attuale.
\end{abstract}
...
Il linguaggio è uno dei fenomeni più complessi e affascinanti dell'intelligenza umana.
Tra i molteplici strumenti che la linguistica computazionale mette a disposizione per
analizzare e manipolare il linguaggio naturale, le grammatiche context-free (CFG) e
gli algoritmi di parsing occupano un ruolo centrale, tanto nella ricerca accademica
quanto nelle applicazioni pratiche come compilatori, assistenti vocali e sistemi di
traduzione automatica.

Questo progetto nasce come esercitazione pratica su tali strumenti, con un obiettivo
volutamente leggero ma tecnicamente significativo: realizzare un convertitore che,
data una frase in italiano con ordine canonico soggetto-verbo-oggetto (SVO), produca
una versione nella stessa lingua ma con un diverso ordine delle costituenti,
ispirato allo stile sintattico del personaggio Yoda dell'universo di Guerre Stellari.

Il risultato è il sistema \texttt{IT-YO} (\textit{Italiano-Yoda}), che opera nelle
seguenti fasi:
\begin{enumerate}
    \item \textbf{Tokenizzazione}: la frase in input viene suddivisa in parole (token).
    \item \textbf{Analisi lessicale}: ogni token viene identificato tramite un lessico
          (\texttt{Lexicon.json}) che associa le parole alle loro categorie grammaticali.
    \item \textbf{Parsing CKY}: l'algoritmo Cocke-Kasami-Younger costruisce l'albero di
          derivazione sintattica a partire dalla grammatica in CNF definita nel file
          \texttt{regole\_binarie.json}.
    \item \textbf{Trasformazione}: l'albero viene riordinato ricorsivamente per
          produrre l'ordine XSV tipico di Yoda.
    \item \textbf{Linearizzazione}: le foglie dell'albero trasformato vengono estratte
          e assemblate nella frase finale in Italiano-Yoda.
\end{enumerate}

Il sistema è interamente scritto in Python 3 e non dipende da librerie NLP esterne;
la grammatica e il lessico sono rappresentati come dati JSON separati dal codice.
...
\begin{itemize}
    \item $V$ è un insieme finito di \textit{variabili} (simboli non-terminali),
          tipicamente rappresentanti categorie sintattiche come NP, VP, S;
    \item $\Sigma$ è un insieme finito di \textit{terminali} (parole del vocabolario),
          con $V \cap \Sigma = \emptyset$;
    \item $R$ è un insieme finito di \textit{regole di produzione} della forma
          $A \rightarrow \alpha$, con $A \in V$ e $\alpha \in (V \cup \Sigma)^*$;
    \item $S \in V$ è il \textit{simbolo iniziale} (assioma), nel nostro caso \texttt{S}.
\end{itemize}
...
struttura sintattica delle frasi: la struttura a costituenti (NP, VP, PP, ecc.)
si modella naturalmente tramite regole di riscrittura.
