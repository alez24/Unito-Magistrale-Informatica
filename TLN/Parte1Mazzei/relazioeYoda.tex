...
Con Numerale & \textit{Vader distrugge due jedi} & Due jedi Vader distrugge & Il sintagma nominale (NP) composto da numerale e nome viene spostato integralmente. \\ \hline
Con Avverbio & \textit{Yoda vede sith qui} & Sith Yoda vede qui & L'avverbio "qui" mantiene la posizione finale secondo la logica del Caso 2. \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Mancanza di Flessione (Lemmatizzazione):} Il sistema fallisce se viene inserita una forma verbale non esplicitamente mappata. Ad esempio, mentre \textit{"Noi abbiamo la pace"} viene riconosciuto, \textit{"Noi avevamo la pace"} produce un errore poiché "avevamo" non figura nel \texttt{Lexicon.json}.
