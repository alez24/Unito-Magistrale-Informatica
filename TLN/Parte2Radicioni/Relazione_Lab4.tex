...
\begin{table}[H]
  \centering
  \caption{Perplexity Laplace (non normalizzata) sulle 8 frasi di test}
  \begin{tabular}{lrrc}
    \toprule
...
\textbf{Nota su una correzione di correttezza}: in una prima versione \texttt{classifica\_loglik} veniva chiamato con i modelli MLE (\texttt{modello\_tweet}, \texttt{modello\_lett}), in contraddizione con la docstring della funzione, che assume uno smoothing di tipo Laplace. Con MLE, qualunque bigramma mai visto in training ha probabilità esattamente $0$, il floor manuale ($\log 10^{-10} \approx -23$) scatta e finisce per dominare l'intera somma: il classificatore diventava di fatto ``chi ha meno bigrammi mai visti vince'' invece di un confronto genuino di verosimiglianza. Usare i modelli Laplace (che non assegnano mai probabilità esattamente nulla) rende il floor un caso limite che in pratica non si attiva più. Si è inoltre normalizzato il punteggio dividendo per il numero di transizioni, per ottenere una log-likelihood media per token confrontabile anche tra frasi di lunghezza diversa.
