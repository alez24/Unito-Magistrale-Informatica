...
Con soglia \texttt{FREQ\_MINIMA=30} si ottengono \textbf{648 parole comuni} analizzate. Un risultato inatteso: per la maggior parte delle parole nel top-30 l'overlap è \textbf{0\%} (cambiamento 100\%) — nessuno dei 10 vicini più prossimi coincide fra le due ere. A dominare questo gruppo sono, sorprendentemente, le parole più \textbf{frequenti in assoluto} del corpus (\textit{love, know, like, got, yeah, get, one, baby, time, want}, \dots, fino a \textit{feel, life, night}): sono le stesse parole ispezionate qualitativamente in Sezione~\ref{sec:parole-simili}. Questo conferma e rafforza quanto osservato in quella sezione: con corpus di poche migliaia di canzoni per era, il vicinato immediato di una parola è dominato da rumore statistico più che da un segnale semantico stabile, e lo è tanto più quanto più la parola è frequente — proprio perché attraversa più contesti diversi, il suo vicinato nei 10 più prossimi si stabilizza meno, non di più. Va quindi interpretato con cautela.

Le parole più \textbf{stabili} (overlap più alto, comunque solo 10--30\%) sono \textit{new} (vicini comuni: \textit{brand, orleans, york}), \textit{rest} (\textit{test, vagabonds}), \textit{single} (\textit{every}), \textit{fate} (\textit{sealed}), \textit{snow} (\textit{covered}), \textit{rich} (\textit{poor}) e \textit{bone} (\textit{flesh}). Guardando questi esempi da vicino emerge un pattern linguistico più interessante della semplice ``stabilità'': ciò che sopravvive intatto per 30 anni non sono temi o significati, ma \textbf{collocazioni e formule fisse della lingua} — \textit{brand new}, \textit{New York}, \textit{New Orleans}, \textit{every single}, \textit{sealed fate}, \textit{snow covered}, \textit{rich or poor}, \textit{flesh and bone}. Il nucleo lessicale che il modello riconosce come invariato nel tempo è quindi prevalentemente \textbf{collocazionale}, non semantico: espressioni multi-parola così frequenti e rigide da restare ancorate le une alle altre indipendentemente dall'era, mentre il resto del vocabolario si muove liberamente.
...
\begin{enumerate}
  ...
  \item \textbf{Il nucleo lessicale più stabile nel tempo è collocazionale, non semantico}: le parole con vicinato più invariato fra le due ere non condividono un tema, ma fanno parte di espressioni fisse della lingua (\textit{brand new}, \textit{every single}, \textit{sealed fate}, Sezione~\ref{sec:vocab-shift}) — ciò che 30 anni di musica non cambiano sono le formule linguistiche congelate, non i significati.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Secondo bug di riproducibilità, più subdolo, in Sezione~\ref{sec:vocab-shift}}: nonostante il fix precedente, un controllo di coerenza fra il testo e il grafico \texttt{word2vec\_drift.png} ha rivelato che le \emph{parole} elencate in top-30 per cambiamento di contesto (e le parole più stabili) cambiavano ad ogni esecuzione, pur restando identici tutti gli altri numeri del laboratorio. Causa: la classifica di Sezione~\ref{sec:vocab-shift} è costruita su un valore quantizzato a soli 11 livelli (l'overlap di 10 vicini), quindi centinaia di parole risultano a pari merito al 100\% di cambiamento; l'ordine con cui Python itera un \texttt{set} (da cui la lista di parole candidate era generata) dipende da \texttt{PYTHONHASHSEED}, randomizzato ad ogni processo, quindi a parità di modello (identico, per il fix precedente) cambiava quale sottoinsieme di parole a pari merito finiva nella top-30 mostrata. Corretto ordinando esplicitamente le liste derivate da insiemi (\texttt{sorted(...)}) e usando come criterio di spareggio la frequenza combinata nelle due ere, cosa che ha anche reso il risultato più interessante: la top-30 non è più un elenco arbitrario, ma è dominata (in modo riproducibile) dalle parole più frequenti del corpus — le stesse di Sezione~\ref{sec:parole-simili} — rafforzando invece di contraddire l'argomento centrale della sezione.
\end{itemize}
