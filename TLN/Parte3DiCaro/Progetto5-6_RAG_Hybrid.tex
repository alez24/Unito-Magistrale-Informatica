...
\begin{table}[H]
  ...
  \begin{tabular}{p{7cm} p{7cm}}
    ...
    How does word2vec represent word meaning?       & word2vec, skip-gram, word embedding, word representation, word vector \\
    What are the main challenges in machine translation? & machine translation, neural machine translation, translation, nmt \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Context Relevance  & 0.716 & 0.715 & $-$0.002 \\
    Answer Relevance   & 0.676 & 0.657 & $-$0.019 \\
    Faithfulness       & 0.503 & 0.740 & +0.237 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Faithfulness       & 0.571 & 0.693 & +0.123 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Context Relevance  & 0.713 & 0.731 & +0.019 \\
    Answer Relevance   & 0.683 & 0.675 & $-$0.008 \\
    Faithfulness       & 0.604 & 0.727 & +0.123 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Faithfulness       & 0.599 & 0.702 & +0.103 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Faithfulness       & 0.438 & 0.635 & +0.197 \\
...
Tutti i risultati precedenti sono calcolati a $k=5$ (numero di documenti passati al retriever/LLM). Per verificare se le conclusioni valgono anche per altri valori di $k$, l'esperimento \`e stato esteso a $k \in \{3, 10, 15\}$, in aggiunta al $k=5$ gi\`a discusso. Precision@$k$ e Context Relevance, che richiedono solo retrieval, sono state misurate per \emph{tutte} le configurazioni di N\_DOCS; Answer Relevance e Faithfulness, che richiedono una generazione dell'LLM per ciascuna combinazione (query, $k$), sono state misurate solo a N\_DOCS=1\,000, per contenere i tempi di esecuzione su CPU (il costo della generazione \`e indipendente da N\_DOCS ma cresce linearmente col numero di valori di $k$ testati).

\begin{table}[H]
  \centering
  \caption{Precision@$k$ e Context Relevance, base vs.\ ibrido, al variare di $k$ e N\_DOCS}
  \small
  \begin{tabular}{rrrrrr}
    \toprule
    \textbf{N\_DOCS} & \textbf{$k$} & \textbf{P@$k$ Base} & \textbf{P@$k$ Ibrido} & \textbf{CR Base} & \textbf{CR Ibrido} \\
    \midrule
    1\,000  & 3  & 0.200 & 0.533 & 0.722 & 0.720 \\
    1\,000  & 5  & 0.160 & 0.480 & 0.716 & 0.715 \\
    1\,000  & 10 & 0.120 & 0.460 & 0.707 & 0.708 \\
    1\,000  & 15 & 0.093 & 0.413 & 0.703 & 0.707 \\
    \midrule
    5\,000  & 3  & 0.267 & 0.733 & 0.725 & 0.733 \\
    5\,000  & 5  & 0.280 & 0.680 & 0.724 & 0.722 \\
    5\,000  & 10 & 0.200 & 0.600 & 0.719 & 0.712 \\
    5\,000  & 15 & 0.200 & 0.560 & 0.714 & 0.714 \\
    \midrule
    15\,000 & 3  & 0.067 & 0.533 & 0.711 & 0.730 \\
    15\,000 & 5  & 0.200 & 0.560 & 0.713 & 0.731 \\
    15\,000 & 10 & 0.280 & 0.580 & 0.716 & 0.716 \\
    15\,000 & 15 & 0.267 & 0.573 & 0.715 & 0.714 \\
    \midrule
    25\,000 & 3  & 0.133 & 0.600 & 0.721 & 0.731 \\
    25\,000 & 5  & 0.200 & 0.560 & 0.720 & 0.728 \\
    25\,000 & 10 & 0.220 & 0.600 & 0.714 & 0.721 \\
    25\,000 & 15 & 0.240 & 0.560 & 0.716 & 0.718 \\
    \midrule
    44\,949 & 3  & 0.067 & 0.467 & 0.723 & 0.721 \\
    44\,949 & 5  & 0.040 & 0.480 & 0.719 & 0.724 \\
    44\,949 & 10 & 0.220 & 0.520 & 0.718 & 0.723 \\
    44\,949 & 15 & 0.187 & 0.547 & 0.715 & 0.720 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Il vantaggio dell'ibrido in Precision@$k$ non \`e un artefatto di $k=5$: l'ibrido supera il base in \textbf{tutte le 20 combinazioni} di N\_DOCS e $k$ testate, con margini che vanno da un fattore ${\sim}2\times$ (es.\ N=25\,000, $k=15$: 0.240 vs.\ 0.560) fino a ${\sim}8\times$ (N=15\,000, $k=3$: 0.067 vs.\ 0.533). Il sistema base risulta inoltre pi\`u instabile al variare di $k$ di quanto suggerisse il solo $k=5$: a N=15\,000, ad esempio, P@3 base (0.067) \`e nettamente pi\`u basso di P@5 base (0.200) sullo stesso corpus, segno che i primissimi risultati del retrieval puramente semantico non sono affidabili quanto quelli, gi\`a pochi, recuperati a $k$ pi\`u alto. L'ibrido \`e invece pi\`u stabile: BM25 ancora i primi posti della fusione a documenti che contengono le parole chiave della query, indipendentemente da quanti se ne richiedano.

La Context Relevance resta sostanzialmente piatta ($0.70$--$0.73$) a ogni $k$, N\_DOCS e sistema: conferma, anche nella variazione di $k$, il limite gi\`a discusso in Sezione~\ref{sec:cr-ar-limite} (CR e AR condividono l'embedding SciBERT con il retrieval semantico).

Sulla qualit\`a della generazione (N\_DOCS=1\,000): l'Answer Relevance dell'ibrido resta leggermente sotto quella del base a \emph{ogni} $k$ testato (delta tra $-0.008$ e $-0.042$), confermando che si tratta di un effetto sistematico del retrieval ibrido e non di una particolarit\`a di $k=5$. La Faithfulness cresce con $k$ per entrambi i sistemi (pi\`u contesto offre pi\`u materiale su cui ancorare le parole della risposta), ma l'ibrido mantiene un vantaggio ampio e consistente a ogni $k$ (delta tra $+0.176$ e $+0.237$, media ${\sim}+0.21$, perfino superiore al delta medio gi\`a osservato a $k=5$ sulle cinque configurazioni di N\_DOCS). In sintesi: la risposta alla domanda ``avete provato altri valori di $k$?'' \`e s\`i, e le conclusioni principali dell'esperimento -- l'ibrido recupera pi\`u documenti rilevanti e produce risposte pi\`u fedeli al contesto, a scapito di una Answer Relevance leggermente pi\`u bassa -- si confermano a ogni $k$ testato, non solo a $k=5$.
...
\begin{itemize}
  ...
  \item \textbf{Faithfulness proxy calcolata per sottostringa}: il confronto \texttt{parola in contesto} usava un test di sottostringa (\texttt{w in context\_text}), che genera falsi positivi (``that'' risulta ``presente nel contesto'' perch\'e \`e sottostringa di ``mathematical''). Inoltre la parola ``paper'' compare sempre sia nelle intestazioni del contesto (\texttt{[Paper N]}) sia nelle citazioni della risposta, quindi viene sempre contata come supportata a prescindere dal contenuto reale. Corretto confrontando insiemi di parole intere ed escludendo esplicitamente ``paper'' da entrambi i lati del confronto.
  \item \textbf{Stopword NLTK potenzialmente assenti senza avviso}: se il corpus \texttt{stopwords} di NLTK non era scaricato, il codice originale procedeva silenziosamente con un insieme vuoto, facendo contare alla Faithfulness anche parole funzionali (``that'', ``with'', ``this''). Corretto scaricando il corpus al volo se mancante, con un avviso esplicito se il download stesso fallisce.
\end{itemize}

Il fix pi\`u importante per il confronto fra dimensioni del corpus \`e il primo (subset annidati): senza di esso, la curva ``Precision@5 al variare di N\_DOCS'' non era interpretabile, perch\'e un calo poteva dipendere solo da quali paper erano finiti per caso in un campione. Con i subset annidati, il crollo del sistema base a N=44\,949 (Sezione~\ref{subsec:crollo-base}) \`e un risultato solido: lo stesso identico corpus, con e senza BM25, produce una differenza di quasi 12 volte in Precision@5.

Gli ultimi due fix (faithfulness per sottostringa e stopword mancanti) riguardano invece solo la metrica di Faithfulness: i valori assoluti riportati in questo documento sono pi\`u bassi di una prima versione del calcolo (che risultava artificialmente gonfiata dai falsi positivi), ma il confronto fra base e ibrido -- l'unico confronto rilevante per le conclusioni -- non cambia di segno in nessuna configurazione: l'ibrido resta sistematicamente pi\`u fedele del base, con margini in alcuni casi anche pi\`u ampi di quelli misurati prima della correzione.
...
\begin{enumerate}
  ...
  \item \textbf{La Faithfulness beneficia chiaramente del retrieval ibrido}: margini fra $+0.103$ e $+0.237$ in tutte le configurazioni, il risultato pi\`u stabile di tutto l'esperimento.
  ...
  \item \textbf{Le conclusioni non dipendono dalla scelta $k=5$}: ripetendo il confronto a $k \in \{3, 10, 15\}$ (Sezione~\ref{sec:variazione-k}), l'ibrido resta pi\`u preciso del base in tutte le 20 combinazioni di N\_DOCS e $k$ testate, e pi\`u fedele al contesto ad ogni $k$ (delta di Faithfulness tra $+0.176$ e $+0.237$ a N=1\,000). L'unico effetto negativo dell'ibrido -- una Answer Relevance leggermente pi\`u bassa -- \`e anch'esso presente ad ogni $k$, confermando che si tratta di una caratteristica sistematica della fusione RRF e non di un artefatto della scelta di $k=5$.
\end{enumerate}
