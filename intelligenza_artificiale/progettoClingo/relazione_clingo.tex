...
\title{
  \Large\textbf{Intelligenza Artificiale -- Progetto Clingo}\\[0.5em]
  \large Pianificazione di Torneo Pokémon con Answer Set Programming\\[0.3em]
  \normalsize Sistema Svizzero -- Relazione Tecnica Dettagliata
}
...
\begin{tikzpicture}[
  ...
  \node[box,fill=blue!15] (ground) {Grounding\\{\footnotesize sostituisce variabili\\con costanti}};
  \node[box,fill=green!15, right=2.5cm of ground] (solve) {Solving\\{\footnotesize cerca \emph{answer sets}\\(modelli stabili)}};
  \node[box,fill=orange!15, right=2.5cm of solve] (opt) {Ottimizzazione\\{\footnotesize seleziona i modelli\\con costo minimo}};
...
\midrule
1 & P appare come P1 in due match diversi & match(emma,diego), match(emma,giada) \\
2 & Q appare come P2 in due match diversi & match(diego,emma), match(giada,emma) \\
3 & P appare in entrambi i ruoli & match(emma,diego), match(giada,emma) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{exbox}{Possibile output Turno 1}
\begin{verbatim}
match(diego,emma)  match(giada,lorenzo)  bye(simone)
\end{verbatim}
oppure qualsiasi altra combinazione di 2 match + 1 bye con giocatori distinti.
\end{exbox}
...
\begin{exbox}{Possibile output ottimale Turno 2}
\begin{verbatim}
match(giada,simone)   match(giada,lorenzo)? ...
\end{verbatim}
Poiché i ``2'' sono 3 (dispari nel loro gruppo), uno verrà abbinato con un ``0'',
contribuendo 2 alla somma totale. L'ottimizzatore minimizza questo inevitabile
"across-group" match.
\end{exbox}
